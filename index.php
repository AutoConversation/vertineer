...
<div class="content">
  <div class="col-1-1">
      <div class="page-title">Vertineer Administration Panel</div>
  </div>
  <div class="col-2-12">
	...
	<?php
    if($_SESSION['id'] == 1 || $_SESSION['id'] == 2 || $_SESSION['id'] == 35) {
      ?>
      <div class="shop-button" req="Logs">Logs</div>
      <?php
    }
    if($bop->isAllowed($localUser->admin, 1)) {
    ?>
  ...
  <div class="col-10-12">
    <div class="card b create">
      <div class="top"><span id="title">Welcome back, <?=htmlentities($user->username)?></span></div>
    </div>
...
function loadAssets() {
	$.get("https://vertineer.com/api/shop/items.php?query=&category=all&sort=&creator=&min=&max=&page=1&perPage=12&showVerified=2", function (data) {
		if (data.status == "ok") {
			$("#content").html("");
			$.each(data.result, function (index,item) {
				console.log(item)
				$("#content").append("<div class='col-1-2'><div class='card border'><div class='col-1-1 centered'><div class='col-1-2'><img src='https://storage.vertineer.com/thumbnails/"+item.id+".png' style='width:100%;'/></div><div class='col-1-2'><img src='https://storage.vertineer.com/juXL6TBLtkhnxHS5/"+item.categoryName+"/"+item.id+".png' style='width:80%;'/></div</div></div></div><div>"+item.name+"</div><div class='col-1-1'><div class='col-1-2'><div style='display:block;' class='button success centered' data-id='"+item.id+"'>Approve</div></div><div class='col-1-2'><div class='button danger centered' style='display:block;' data-id='"+item.id+"'>Decline</div></div></div></div></div>");
			});
		} else {
			$("#content").html("<div class='banner danger centered'>"+data.error+"</div>");
		}
